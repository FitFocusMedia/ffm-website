  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-white">Follow-Up Scheduler</h2>
          <p className="text-gray-400 text-xs sm:text-sm mt-1">5-Touch outreach strategy tracker</p>
        </div>
        
        {/* Summary Stats */}
        <div className="flex gap-4 sm:gap-4">
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold text-red-500">{overdueCount}</div>
            <div className="text-xs text-gray-400">Overdue</div>
          </div>
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold text-yellow-500">{todayCount}</div>
            <div className="text-xs text-gray-400">Today</div>
          </div>
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold text-blue-500">{soonCount}</div>
      ...
      {filteredLeads.length === 0 ? (
        <div className="bg-gray-800 rounded-lg p-6 sm:p-8 text-center text-gray-400">
          <CheckCircle className="w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-2 sm:mb-3 text-green-500" />
          <p className="text-sm sm:text-base">No follow-ups {filter !== 'all' ? filter : 'needed'}!</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filteredLeads.map(lead => (
            <div
              key={lead.id}
              className={`bg-gray-800 rounded-lg p-3 sm:p-4 border-l-4 ${
              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 sm:gap-3 mb-2 flex-wrap">
                    <h3 className="text-base sm:text-lg font-bold text-white truncate">{lead.org_name}</h3>
                    <span className={`px-2 py-0.5 sm:py-1 rounded text-xs font-medium flex-shrink-0 ${
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-xs sm:text-sm text-gray-400">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="truncate">Touch {lead.nextTouch.touch}: {lead.nextTouch.name}</span>
                    </div>
                ...
                <div className="flex items-center gap-2 self-end sm:self-auto">
                  {lead.status === 'overdue' && (
                    <AlertCircle className="w-5 h-5 text-red-500 hidden sm:block" />
                  )}
                  <button className="bg-red-600 hover:bg-red-700 text-white px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all whitespace-nowrap">
                    Send Follow-Up
                  </button>
                </div>
              </div>
      ...
      {/* Legend - collapsible on mobile for cleaner view */}
      <details className="bg-gray-800 rounded-lg">
        <summary className="p-3 sm:p-4 text-xs sm:text-sm font-bold text-white cursor-pointer list-none flex items-center justify-between">
          5-Touch Strategy Timeline
        ...
        <div className="px-3 sm:px-4 pb-3 sm:pb-4 space-y-2 text-xs sm:text-sm">
          {TOUCH_SCHEDULE.map((touch, idx) => (
            <div key={touch.touch} className="flex items-center gap-2 sm:gap-3 text-gray-400">
              <span className="font-mono text-[10px] sm:text-xs bg-gray-700 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded">T{touch.touch}</span>
              <span className="flex-1 text-xs sm:text-sm">{touch.name}</span>
              <span className="text-[10px] sm:text-xs">
                {idx === 0 ? 'Initial' : `+${touch.daysAfterPrevious} days`}
              </span>
            </div>
          ))}
          <div className="mt-3 pt-3 border-t border-gray-700 text-[10px] sm:text-xs text-gray-500">
